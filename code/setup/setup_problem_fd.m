function [problem_data, method_data] = setup_problem_fd (geometry_file, omega)
   % magnetic problem
   ...
   mu22  = 1.012e-5;
   sigma = 30e6;
   % epsilon only relevant in full maxwell (time domain)
   epsilon = 0;

   problem_data.mu    = {@(x,y,iptc) compute_mu(x, y, iptc, mu11), @(x,y,iptc) compute_mu(x, y, iptc, mu22)};
   problem_data.kappa = {@(x,y,iptc) compute_kappa(x, y, iptc, omega, sigma, epsilon)};
