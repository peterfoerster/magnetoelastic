...
function varargout = op_mqs2d (spA, spAt, msh, mu, kappa)
   valA  = reshape (spA.shape_functions, [], msh.nqn, spA.nsh_max, msh.nel);
   valAt = reshape (spAt.shape_functions, [], msh.nqn, spAt.nsh_max, msh.nel);

   gradA  = reshape (spA.shape_function_gradients, spA.ncomp, [], msh.nqn, spA.nsh_max, msh.nel);
   gradAt = reshape (spAt.shape_function_gradients, spAt.ncomp, [], msh.nqn, spAt.nsh_max, msh.nel);
   ...
   for iel=1:msh.nel
      if (all(msh.jacdet(:,iel)))
         % (nqn x 1 x nsh)
         valA_iel  = reshape(valA(:,:,1:spA.nsh(iel),iel), 1, msh.nqn, 1, spA.nsh(iel));
         valAt_iel = reshape(valAt(:,:,1:spAt.nsh(iel),iel), 1, msh.nqn, spAt.nsh(iel), 1);

         % (ndim x nqn x 1 x nsh)
         ...
         kappa_iel  = reshape(kappa{1}(:,iel), [1,msh.nqn,1,1]);
         ...
         tmp3 = bsxfun (@times, jacdet_iel.*kappa_iel, valA_iel(1,:,:,:));
         tmp3 = bsxfun (@times, tmp3, valAt_iel(1,:,:,:));

         values(ncounter+(1:spA.nsh(iel)*spAt.nsh(iel))) = reshape(sum(tmp1 + tmp2 + tmp3, 2), spAt.nsh(iel), spA.nsh(iel));
         ...
         warning('geopdes:jacdet_zero_at_quad_node', 'op_mqs2d: singular map in element number %d', iel)
      end
   end

   if (nargout == 1 || nargout == 0)
      varargout{1} = sparse(rows(1:ncounter), cols(1:ncounter), values(1:ncounter), spAt.ndof, spA.ndof);
   elseif (nargout == 3)
      varargout{1} = rows(1:ncounter);
      varargout{2} = cols(1:ncounter);
      varargout{3} = values(1:ncounter);
   else
      error ('op_mqs2d: wrong number of output arguments')
   end
end
