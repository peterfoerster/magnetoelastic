...
tic;
% [geometry_mec, msh_mec, space_mec, u_fd, msh_mag, space_mag, A] = mp_solve_coupling2d_fd (problem_data, method_data);
...
% mechanic problem
if (mechanic)
   save(['u_fd_degree=' num2str(method_data.degree(1)) '_nsub=' num2str(method_data.nsub(1)) '.mat'], 'u_fd');
   i_cmp = complex(0, 1);
   u_td = u_fd - u;
   for it=1:length(T)
      % subtract preload first?
      u_plot = (u + real(u_td * exp(i_cmp*omega*T(it)))*1e1) * 1e3;
      figure(it);
